<template>
  <scroll-view class="waterfall-container" scrollbar="true" scroll-y="true" @scrolltolower="loadDataList">
    <view class="waterfall-list">
      <view v-if="isShow" v-for="item in renderList" :key="item.articleId" class="waterfall-item">
        <card :src="item.avatar" :title="item.title" :author="item.userName" :bgColor="item.bgColor"
          :content="item.content" :viewCount="item.viewCount" @toDetail="handleToDetail(item.articleId)"></card>
      </view>
...
export default {
  ...
  computed: {
    lastId() {
      return this.renderList.length !== 0 ? this.renderList[this.renderList.length - 1]?.articleId : null
    },
    // hasMoreData(){
