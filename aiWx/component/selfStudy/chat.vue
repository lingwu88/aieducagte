<template>
	<view class="chat-container" :class="[{ 'char-container-minHeight': show }]">
		<!-- 聊天内容区域  
			...
			scroll-top 竖直滚动条的位置
		-->
		<scroll-view class="chat-content" scroll-y scroll-top="scrollTop" :scroll-into-view="lastMessageId" @scrolltoupper="loadMoreHistory">
			<!-- 加载更多 -->
			...
			<view class="message-list">
				<view v-for="(item, index) in messageList" :key="item.id" :id="'msg-' + index" class="message-item" :class="item.type">
					<!-- 头像 -->
					<image :src="item.type === 'user' ? userAvatar : aiAvatar" class="avatar" />
					<!-- 消息内容 -->
            ...
            /> -->
						...
						<!-- 语音消息 -->
						<view v-else-if="item.contentType === 'voice'" class="voice-content" @tap="playVoice(item.content)">
	...
	methods: {
		...
		initRequest() {
			...
			this.requestbody = {
				...
				courses: body.courses.map((item) => item.desc)
			};
		...
		async handleSend() {
			...
			if (Object.values(this.setting).some((value) => value === '' || value === null || value === undefined)) {
				uni.showToast({
					title: '请填写完设置!',
					icon: 'error'
				});
		...
		closeSSE() {
			this.$api.classManagement
				.endSSE(uni.getStorageSync('userId'))
				.then((res) => {
					console.log(res);
					console.log('关闭');
