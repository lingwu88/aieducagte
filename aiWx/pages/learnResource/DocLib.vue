      ...
      <block v-for="item in resources" :key="item.id">
        <view v-if="item.url" class="resource-item" @click="navigateTo(item.url)">
          ...
          <view class="item-content">
            <text class="item-title" :class="{ ellipsis: item.title.length > maxTitleLength }">{{ item.title || '无标题' }}</text>
            <text class="item-desc" :class="{ 'ellipsis-two': item.preview && item.preview.length > maxPreviewLength }">{{ item.preview || '无描述' }}</text>
          </view>
...
export default {
  ...
  methods: {
    ...
    loadStaticData() {
      const staticData = [
        { id: 1, title: 'AI基础教程', preview: '学习AI的基本概念', img: '/static/classroom/learnResource/res1.jpg', url: 'https://www.datawhale.cn/learn/content/30/1833', category: '人工智能' },
        { id: 2, title: '深度学习入门', preview: '深度学习基础', img: '/static/classroom/learnResource/res2.jpg', url: 'https://www.datawhale.cn/learn/content/30/1833', category: '人工智能' },
